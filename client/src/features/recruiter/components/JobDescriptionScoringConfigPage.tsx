              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-semibold text-slate-500">
                    {k >= 1 && k <= 25 ? `${k} Candidates` : "Invalid Size"}
                  </span>
                </div>
                <input
                  id="k-value"
                  ...
                  onBlur={(e) => {
                    ...
                    setK(val);
                    localStorage.setItem("recruiter_preferred_k", String(val));
                  }}
                  className="auth-input !py-2.5 text-sm"
                />
                {(k < 1 || k > 25 || isNaN(k)) && (
                  <div className="rounded-xl border border-amber-200 bg-amber-50/95 p-3.5 text-xs text-amber-900 flex items-start gap-2.5 mt-2 shadow-[0_4px_12px_rgba(180,83,9,0.05)] animate-fade-in">
                    <svg className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    <div className="space-y-1">
                      <div className="font-bold flex items-center gap-1">⚠️ Invalid Shortlist Size</div>
                      <p className="leading-relaxed">
                        The final shortlist size must be between 1 and 25 candidates. Please enter a valid value before continuing.
                      </p>
                    </div>
                  </div>
                )}
