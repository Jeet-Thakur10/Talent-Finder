...
export function LoginPage() {
  ...
  // Determine active step from current URL path
  const getActiveStep = (path: string) => {
    if (path === "/forgot-password") return 1;
    if (path === "/verify-otp") return 2;
    if (path === "/reset-password") return 3;
    return 0; // Default to login
  };

  const activeStep = getActiveStep(location.pathname);

  // States for Login (Step 0)
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [loginError, setLoginError] = useState("");
  const [isSubmittingLogin, setIsSubmittingLogin] = useState(false);

  // States for Forgot Password (Step 1)
  const [forgotEmail, setForgotEmail] = useState("");
  const [forgotError, setForgotError] = useState("");
  const [isSubmittingForgot, setIsSubmittingForgot] = useState(false);

  // States for Verify OTP (Step 2)
  const [otp, setOtp] = useState("");
  const [otpError, setOtpError] = useState("");
  const [isSubmittingOtp, setIsSubmittingOtp] = useState(false);

  // States for Reset Password (Step 3)
  ...
  // Retrieve email and reset token passed in location state
  const stateEmail = (location.state as { email?: string })?.email || forgotEmail;
  const resetToken = (location.state as { resetToken?: string })?.resetToken;

  // Route protection/redirect guards in active state
  useEffect(() => {
    if (location.pathname === "/verify-otp" && !stateEmail) {
      navigate("/forgot-password", { replace: true });
    }
    if (location.pathname === "/reset-password" && !resetToken) {
      navigate("/forgot-password", { replace: true });
    }
  }, [location.pathname, stateEmail, resetToken, navigate]);
  ...
  const handleForgotSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setForgotError("Unable to send OTP. Please try again.");
    } finally {
  ...
  return (
    <div className="w-screen h-screen overflow-hidden relative bg-[#BDDDFC] font-sans">
      {/* Background design elements that span across the viewport */}
      <div className="absolute -top-20 -left-20 w-[60vw] h-[60vw] bg-[#6A89A7]/25 rounded-full blur-[140px] pointer-events-none z-0" />
      <div className="absolute bottom-0 right-0 w-[50vw] h-[50vw] bg-[#88BDF2]/20 rounded-full blur-[120px] pointer-events-none z-0" />

      {/* Decorative background lines */}
      <svg className="absolute inset-0 w-full h-full stroke-[#6A89A7]/15 pointer-events-none z-0" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Full-Page Horizontal Slider */}
      <div 
        className="flex h-full transition-transform duration-300 ease-in-out z-10 relative"
        style={{ 
          transform: `translate3d(-${activeStep * 25}%, 0, 0)`,
          width: "400vw" 
        }}
      >
        {/* PAGE 0: Login (Split-Screen Layout) */}
        <div className="w-screen h-full grid grid-cols-1 md:grid-cols-12 shrink-0 overflow-y-auto bg-transparent">
          
          {/* Left Column: Dark Slate, illustration, copy */}
          <div className="hidden md:flex md:col-span-5 flex-col justify-between p-8 lg:p-12 xl:p-14 relative bg-[#384959] border-r border-[#384959]/20 min-h-full">
            ...
            {/* Hero copy and illustration container */}
            <div className="flex-1 flex flex-col justify-center py-4 z-10">
              <div className={`transition-all duration-700 delay-200 ${
                ...
                <p className="text-[#BDDDFC]/85 text-xs lg:text-sm leading-relaxed mt-3 max-w-sm">
                  Use AI-powered sourcing and intelligent candidate evaluation to discover your next hire faster.
                </p>
              </div>
          ...
          <div className="col-span-12 md:col-span-7 flex flex-col items-center justify-center p-6 sm:p-10 lg:p-14 relative bg-[#BDDDFC]/45 backdrop-blur-[2px] min-h-full">
            <div className="w-full max-w-[440px]">
              ...
              <div className="w-full bg-white border border-[#6A89A7]/20 rounded-[32px] p-8 sm:p-12 shadow-[0_28px_60px_-15px_rgba(56,73,89,0.15)]">
                <div className="mb-7">
                  <h3 className="text-xl font-bold text-[#384959]">Welcome Back</h3>
                ...
                <form onSubmit={handleLoginSubmit} className="space-y-6">
                  ...
                  <div className="space-y-1.5">
                    <label htmlFor="password" className="text-xs font-bold text-[#384959] block">
                      Password
                    </label>
                    <div className="relative">
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#88BDF2] pointer-events-none">
                ...
                <div className="mt-7 border-t border-slate-100 pt-5 text-center">
                  <Link to="/forgot-password" className="text-xs font-semibold text-[#6A89A7] hover:text-[#384959] transition-colors">
                    Forgot password?
                  </Link>
                </div>
              </div>
        ...
        {/* PAGE 1: Forgot Password (Full-Width, Centered Card, No Left Panel, No Illustration) */}
        <div className="w-screen h-full flex flex-col items-center justify-center p-6 sm:p-10 lg:p-14 shrink-0 overflow-y-auto bg-[#BDDDFC]/45 backdrop-blur-[2px]">
          <div className="w-full max-w-[440px]">
            {/* Centered logo */}
            <div className="flex flex-col items-center text-center mb-8">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-[#6A89A7] to-[#88BDF2] text-white shadow-md shadow-[#6A89A7]/10 mb-3.5 hover:scale-105 transition-transform duration-200">
            ...
            <div className="w-full bg-white border border-[#6A89A7]/20 rounded-[32px] p-8 sm:p-12 shadow-[0_28px_60px_-15px_rgba(56,73,89,0.15)]">
              <div className="mb-7">
                <h3 className="text-xl font-bold text-[#384959]">Reset Password</h3>
                <p className="text-xs text-[#6A89A7] mt-1">
              ...
              <form onSubmit={handleForgotSubmit} className="space-y-6">
                <div className="space-y-1.5">
                  <label htmlFor="forgot-email" className="text-xs font-bold text-[#384959] block">
                    Email Address
                  </label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#88BDF2] pointer-events-none">
        ...
        {/* PAGE 2: Verify OTP (Full-Width, Centered Card, No Left Panel, No Illustration) */}
        <div className="w-screen h-full flex flex-col items-center justify-center p-6 sm:p-10 lg:p-14 shrink-0 overflow-y-auto bg-[#BDDDFC]/45 backdrop-blur-[2px]">
          <div className="w-full max-w-[440px]">
            {/* Centered logo */}
            <div className="flex flex-col items-center text-center mb-8">
            ...
            <div className="w-full bg-white border border-[#6A89A7]/20 rounded-[32px] p-8 sm:p-12 shadow-[0_28px_60px_-15px_rgba(56,73,89,0.15)]">
              <div className="mb-7">
                <h3 className="text-xl font-bold text-[#384959]">Verify Code</h3>
                <p className="text-xs text-[#6A89A7] mt-1">
              ...
              <form onSubmit={handleOtpSubmit} className="space-y-6">
                <div className="space-y-1.5">
                  <label htmlFor="otp" className="text-xs font-bold text-[#384959] block">
        ...
        {/* PAGE 3: Reset Password (Full-Width, Centered Card, No Left Panel, No Illustration) */}
        <div className="w-screen h-full flex flex-col items-center justify-center p-6 sm:p-10 lg:p-14 shrink-0 overflow-y-auto bg-[#BDDDFC]/45 backdrop-blur-[2px]">
          <div className="w-full max-w-[440px]">
            {/* Centered logo */}
            <div className="flex flex-col items-center text-center mb-8">
            ...
            <div className="w-full bg-white border border-[#6A89A7]/20 rounded-[32px] p-8 sm:p-12 shadow-[0_28px_60px_-15px_rgba(56,73,89,0.15)]">
              <div className="mb-7">
                <h3 className="text-xl font-bold text-[#384959]">Choose a New Password</h3>
                <p className="text-xs text-[#6A89A7] mt-1">
              ...
              <form onSubmit={handleResetSubmit} className="space-y-6">
                <div className="space-y-1.5">
                  <label htmlFor="new-password" className="text-xs font-bold text-[#384959] block">
                    New Password
                  </label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#88BDF2] pointer-events-none">
