...
async def run_verification():
    ...
    async with async_session_local() as db:
        ...
        CandidateSearchClient.search_candidates = lambda self, req: mock_search_client.search_candidates(req)
        CandidateSearchClient.get_candidate_details = lambda self, ids: mock_search_client.get_candidate_details(ids)

        original_prescore = CandidatePrescoringClient.prescore_candidates
        CandidatePrescoringClient.prescore_candidates = AsyncMock(return_value=mock_prescores)

        original_score = CandidateScoringClient.score_candidate
        CandidateScoringClient.score_candidate = AsyncMock(side_effect=lambda job, cand: mock_deep_score(cand.candidate_id))
        ...
        try:
            ...
            print(f"Task created in DB: ID={task.id}, status={task.status}, stage={task.current_stage}")
            ...
            print(f"Task state after run: status={task.status}, stage={task.current_stage}")
            print(f"Summary counts: matched={task.matched_candidate_count}, eligible={task.eligible_candidate_count}, selected={task.selected_candidate_count}")
            ...
            CandidatePrescoringClient.prescore_candidates.side_effect = Exception("Groq AI Timeout Exception")
            ...
            print(f"Task 2 state after failure: status={task2.status}, stage={task2.current_stage}")
