...
async def async_run_scoring_pipeline(
    ...
    async with async_session_local() as db:
        ...
        try:
            ...
            try:
                user_res = await db.execute(select(User).where(User.id == recruiter_id))
                ...
                if user and jd:
                    ...
                    await notification_service.notify(
                        ...
                        metadata={"job_description_id": str(job_description_id), "task_id": str(task_id)},
                        send_in_app=True,
                        send_email=True,
                        email_subject=f"Scoring completed for {jd.title}",
                        email_html=email_html
                    )
                    logger.info("Pipeline completed notification sent successfully for task %s", task_id)
                else:
                    logger.warning("Could not send pipeline success notification: User or JobDescription not found.")
            except Exception as ne:
                logger.warning("Failed to send success notification for task %s: %s", task_id, str(ne), exc_info=True)

        except Exception as e:
            logger.exception("Error executing background scoring task %s: %s", task_id, str(e))
            import traceback
            error_message = "".join(traceback.format_exception(type(e), e, e.__traceback__))
            # We rollback the current transaction in case of SQL errors, then log the failure in a fresh transaction block
            await db.rollback()
            await task_service.update_task_failure(task_id, error_message)
            await db.commit()

            # 4. Best-effort failure notification
            try:
                user_res = await db.execute(select(User).where(User.id == recruiter_id))
                user = user_res.scalar_one_or_none()
                ...
                if user and jd:
                    ...
                    logger.info("Pipeline failure notification sent successfully for task %s", task_id)
                else:
                    logger.warning("Could not send pipeline failure notification: User or JobDescription not found.")
            except Exception as ne:
                logger.warning("Failed to send failure notification for task %s: %s", task_id, str(ne), exc_info=True)
