...
async def main():
    print(
        "Testing update flow and candidate scoring with an extracted Job Description..."
    )
    sample_jd = """
    ...
    async with async_session_local() as db:
        jd_service = JobDescriptionService(db)
        updated_jd_res = await jd_service.update_job_description(
            saved_jd.id, update_req, current_user
        )
    ...
    async with async_session_local() as db:
        ...
        print(
            f"Stored raw_job_description changed: {db_jd.raw_job_description == 'THIS DUMMY TEXT SHOULD BE COMPLETELY IGNORED BY THE UPDATE SERVICE AND ENDPOINT'}"
        )
        print(
            f"Stored raw_job_description matches original: {db_jd.raw_job_description == sample_jd}"
        )
        ...
        assert db_jd.raw_job_description == sample_jd, (
            "Raw job description was incorrectly updated!"
        )
        print(
            "Success! Structured fields updated successfully, while original raw_job_description was preserved exactly!"
        )
