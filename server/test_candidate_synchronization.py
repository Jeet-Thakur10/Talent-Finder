...
async def run_verification():
    ...
    async with async_session_local() as db:
        ...
        assert mock_client.called_with_ids == [[C2_ID]], f"Expected sync call for [C2_ID], got {mock_client.called_with_ids}"
        ...
        print(f"HTTP requests made: {mock_client.called_with_ids}")
        assert len(mock_client.called_with_ids) == 0, f"Expected no sync calls, got {mock_client.called_with_ids}"
        print("Case 2 PASSED!")
        ...
        # ====================================================
        print("\n" + "="*70)
        print("CASE 3: Candidate exists locally and is stale (Older than threshold, Refresh)")
        print("="*70)

        # Pre-seed C3 as stale (updated 45 days ago)
        stale_time = now - timedelta(days=45)
        stale_details = make_mock_details(C3_ID, "Candidate Three", updated_at=stale_time)
        ...
        mock_client.mock_details[C3_ID] = make_mock_details(C3_ID, "Candidate Three (Refreshed)")

        await sync_service.synchronize_candidates([C3_ID])
        await db.commit()

        print(f"HTTP requests made: {mock_client.called_with_ids}")
        assert mock_client.called_with_ids == [[C3_ID]], f"Expected sync call for [C3_ID], got {mock_client.called_with_ids}"
        ...
        assert cand.full_name == "Candidate Three (Refreshed)", "Candidate was not updated via UPSERT!"
        print("Case 3 PASSED!")
        ...
        fresh_details = make_mock_details(C1_ID, "Candidate One", updated_at=now - timedelta(days=10))
        await scoring_service.upsert_candidate_profile(fresh_details)
        cand_c1 = await scoring_service.repository.get_candidate_by_id(C1_ID)
        cand_c1.updated_at = now - timedelta(days=10)

        # Pre-seed C3 as stale
        stale_details = make_mock_details(C3_ID, "Candidate Three", updated_at=now - timedelta(days=45))
        await scoring_service.upsert_candidate_profile(stale_details)
        cand_c3 = await scoring_service.repository.get_candidate_by_id(C3_ID)
        ...
        assert requested_set == {C2_ID, C3_ID}, f"Expected request for {C2_ID, C3_ID}, got {requested_set}"
