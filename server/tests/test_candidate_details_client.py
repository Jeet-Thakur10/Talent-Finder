...
async def seed_sourcing_database():
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(
            text(
                "DELETE FROM candidate_experience_skills WHERE experience_id = :exp_id"
            ),
            {"exp_id": EXPERIENCE_ID},
        )
        await conn.execute(
            text(
                "DELETE FROM candidate_experiences WHERE candidate_id = :candidate_id"
            ),
    ...
    async with CandidateSearchClient() as client:
        ...
        try:
            ...
            assert candidate.id == CANDIDATE_ID, (
                f"Expected ID {CANDIDATE_ID}, got {candidate.id}"
            )
            ...
            assert (
                candidate.summary
                == "An experienced software engineer specializing in Python and PostgreSQL."
            )
            ...
            assert (
                exp.description
                == "Built FastAPI applications and optimized Postgres database queries."
            )
