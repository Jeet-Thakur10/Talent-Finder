...
async def main():
    ...
    c3 = Candidate(
        ...
        updated_at=now,
        skills=[
            CandidateSkill(skill_name="Java", is_primary=True)
        ]
    )
    ...
    async with async_session_local() as db:
        ...
        print(f"Extraction complete. Title: {extract_res.title}, Min Experience: {extract_res.min_experience}")
    ...
    try:
        ...
        preview_req = PipelineExecutionRequest(confirm=False, k=5, minimum_prescore_threshold=60)
        async with async_session_local() as db:
        ...
        case_c_req = PipelineExecutionRequest(confirm=True, k=5, minimum_prescore_threshold=0)
        async with async_session_local() as db:
            scoring_service = ScoringService(db)
        ...
        case_a_req = PipelineExecutionRequest(confirm=True, k=5, minimum_prescore_threshold=threshold_a)
        async with async_session_local() as db:
            scoring_service = ScoringService(db)
            ...
            expected_eligible_count = sum(1 for name, score in prescores.items() if score >= threshold_a)
        ...
        case_b_req = PipelineExecutionRequest(confirm=True, k=5, minimum_prescore_threshold=100)
        async with async_session_local() as db:
            scoring_service = ScoringService(db)
        ...
        async with async_session_local() as db:
            ...
            await db.execute(delete(CandidateSkill).where(CandidateSkill.candidate_id.in_([c1_id, c2_id, c3_id])))
            await db.execute(delete(Candidate).where(Candidate.id.in_([c1_id, c2_id, c3_id])))
