...
async def clear_database(db):
    test_ids = [C1_ID, C2_ID, C3_ID] + [
        UUID(f"90000000-0000-0000-0000-0000000000{10 + i:02d}") for i in range(12)
    ]
    for cid in test_ids:
        await db.execute(
...
async def run_verification():
    ...
    async with async_session_local() as db:
        ...
        mock_scoring_client.score_candidate.side_effect = lambda job, cand: (
            mock_deep_score(cand.candidate_id)
        )
        ...
        for i in range(12):
            ...
            await scoring_service.upsert_candidate_profile(
                make_mock_details(dummy_id, f"Dummy {i}")
            )
        await scoring_service.upsert_candidate_profile(
            mock_search_client.mock_details[C1_ID]
        )
        await scoring_service.upsert_candidate_profile(
            mock_search_client.mock_details[C2_ID]
        )
        await scoring_service.upsert_candidate_profile(
            mock_search_client.mock_details[C3_ID]
        )
        await db.commit()

        local_cands_list = (
            await scoring_service.repository.get_candidates_for_job_description()
        )
        ...
        res1 = await scoring_service.pipeline_prescore_and_score(
            jd_id, current_user, req1
        )
        await db.commit()
        ...
        print("- sourced candidate count:      0")
        ...
        print("- synchronized candidate count: 0")
        ...
        assert len(mock_search_client.details_calls) == 0, (
            "Sync made external details request!"
        )
        ...
        res2 = await scoring_service.pipeline_prescore_and_score(
            jd_id, current_user, req2
        )
        await db.commit()
        ...
        print(f"- local candidate count:        {local_cand_count}")
        print(
            "- sourced candidate count:      3"
        )  # we returned 3 summaries from client
        print("- acquisition result size:      3")  # merged and deduplicated
        print("- pre-score count:              3")
        ...
        print("- synchronized candidate count: 1")  # only C3 was missing and selected
        print(f"- deep-scored candidate count:  {len(res2.candidates)}")

        assert len(mock_search_client.search_calls) == 1, (
            "Sourcing should have been triggered"
        )
        assert mock_search_client.details_calls == [[C3_ID]], (
            f"Expected sync details request only for C3, got {mock_search_client.details_calls}"
        )
        print("\nCASE 2 PASSED!")
        ...
        res3 = await scoring_service.pipeline_prescore_and_score(
            jd_id, current_user, req3
        )
        await db.commit()
        ...
        print("\nVerification Variables:")
        print(f"- local candidate count:        {local_cand_count}")
        print("- sourced candidate count:      3")
        print("- acquisition result size:      3")
        print("- pre-score count:              3")
        ...
        print(
            "- synchronized candidate count: 3"
        )  # C1, C2, and C3 were missing and selected
        ...
        flat_details_requests = [
            cid for req in mock_search_client.details_calls for cid in req
        ]
        assert set(flat_details_requests) == {C1_ID, C2_ID, C3_ID}, (
            f"Expected sync details request for C1, C2, C3, got {flat_details_requests}"
        )
        print("\nCASE 3 PASSED!")
